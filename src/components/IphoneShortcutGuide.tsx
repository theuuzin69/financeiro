...
export function IphoneShortcutGuide({ webhookSecret, onSimulateSuccess }: IphoneShortcutGuideProps) {
  ...
  return (
    <div className="bg-[#121216] border border-zinc-800/80 rounded-3xl p-5 shadow-lg space-y-6">
      {/* Cabeçalho */}
      <div>
        ...
        <p className="text-xs text-zinc-400 mt-1 leading-relaxed">
          Como você não recebe SMS do Santander, estruturamos os canais automáticos ideais: o <strong className="text-zinc-200">Apple Pay</strong> (100% silencioso), <strong className="text-zinc-200">Notificações por E-mail</strong> do Santander, ou o <strong className="text-zinc-200">Atalho Rápido por Voz/Widget</strong>.
        </p>
      </div>

      {/* Seletor de Abas das Opções */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-2xl bg-zinc-900 p-1 border border-zinc-800 text-xs">
        <button
        ...
        <button
          ...
          className={`py-2 px-2 rounded-xl font-bold flex items-center justify-center gap-1 transition-all ${
            activeTab === 'email'
              ? 'bg-santander-red text-white shadow'
              : 'text-zinc-400 hover:text-white'
          }`}
        >
      ...
      {activeTab === 'apple_pay' && (
        <div className="space-y-4">
          <div className="bg-emerald-950/20 border border-emerald-800/40 p-3.5 rounded-2xl text-xs text-emerald-200">
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 p-4 rounded-2xl space-y-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-zinc-400">URL do Webhook Apple Pay:</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={applePayUrl}
                  className="bg-black/50 border border-zinc-800 text-xs text-zinc-300 px-3 py-2 rounded-xl flex-1 font-mono select-all"
                />
                <button
                  onClick={() => copyToClipboard(applePayUrl, 'apple-pay-url')}
                  className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 px-3 py-2 rounded-xl text-xs flex items-center gap-1 font-semibold border border-zinc-700"
                >
                  {copiedUrl === 'apple-pay-url' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                  Copiar
                </button>
              </div>
            </div>

            <div className="bg-black/40 p-3 rounded-xl border border-zinc-800/80 space-y-1 text-xs">
              <div className="flex justify-between text-zinc-400">
                <span>Método:</span> <strong className="text-white">POST</strong>
              </div>
              <div className="flex justify-between text-zinc-400">
                <span>Cabeçalho:</span> <strong className="text-white">Content-Type: application/json</strong>
              </div>
              <div className="mt-2 text-zinc-300">
                <span className="font-semibold block text-zinc-400 mb-1">Corpo da Requisição (JSON):</span>
                <pre className="bg-zinc-950 p-2.5 rounded-lg text-[11px] text-emerald-300 font-mono overflow-x-auto">
{`{
  ...
  "secret": "${webhookSecret}"
      ...
      {activeTab === 'email' && (
        <div className="space-y-4">
          <div className="bg-rose-950/20 border border-rose-800/40 p-3.5 rounded-2xl text-xs text-rose-200">
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 p-4 rounded-2xl space-y-3">
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                ...
                className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 px-3 py-2 rounded-xl text-xs flex items-center gap-1 font-semibold border border-zinc-700"
              >
                {copiedUrl === 'santander-url' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                Copiar
              </button>
            </div>
          </div>
      ...
      {activeTab === 'siri_widget' && (
        <div className="space-y-4">
          <div className="bg-indigo-950/20 border border-indigo-800/40 p-3.5 rounded-2xl text-xs text-indigo-200">
      ...
      {activeTab === 'open_finance' && (
        <div className="space-y-4">
          <div className="bg-teal-950/20 border border-teal-800/40 p-3.5 rounded-2xl text-xs text-teal-200">
      ...
      <div className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 p-4 rounded-2xl space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
          <button
            ...
            className="p-3 rounded-xl bg-zinc-800/90 hover:bg-zinc-700 text-left border border-zinc-700 transition-all group flex flex-col justify-between"
          >
            <span className="text-[10px] text-emerald-400 font-semibold flex items-center gap-1">
              <Smartphone className="w-3 h-3" /> Testar Apple Pay
            </span>
            <span className="text-xs font-bold text-white mt-1 group-hover:text-emerald-300">
              Starbucks (R$ 24,50)
            </span>
            <span className="text-[10px] text-zinc-400 mt-0.5">Alimentação</span>
          </button>
          ...
          >
            ...
            <span className="text-xs font-bold text-white mt-1 group-hover:text-teal-300">
              Restaurante (R$ 68,00)
            </span>
            <span className="text-[10px] text-zinc-400 mt-0.5">Alimentação</span>
          </button>
          ...
          >
            ...
            <span className="text-xs font-bold text-white mt-1 group-hover:text-rose-300">
              Posto Ipiranga (R$ 139,90)
            </span>
            <span className="text-[10px] text-zinc-400 mt-0.5">Transporte</span>
          </button>
        </div>
      ...
      {/* Guia de Publicação na Nuvem (Vercel) */}
      <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-2">
        <h4 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
          <Cloud className="w-3.5 h-3.5 text-sky-400" />
          Hospedagem Gratuita na Nuvem (Vercel)
        </h4>
        <p className="text-xs text-zinc-400 leading-relaxed">
          Para que as automações do iPhone funcionem na rua (fora do Wi-Fi da sua casa), este projeto pode ser publicado na <strong>Vercel</strong> em menos de 2 minutos de forma 100% gratuita. Basta subir para o GitHub e conectar na Vercel!
        </p>
      </div>
    </div>
  );
